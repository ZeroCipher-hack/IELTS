...
export type Result={correct:number;total:number;band:number|null;rows:Row[];skills:Record<string,{correct:number;total:number}>;weekly_plan?:Plan[]};
export type Attempt={id:string;title:string;section:string;state:string;deadline:string;server_time:string;started_at:string;answers:Record<string,string>;review_positions:number[];passage?:string;audio_url?:string;questions?:{position:number;prompt:string;choices:string[];skill_tag:string}[];result:Result|null};
