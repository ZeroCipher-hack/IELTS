...
export type Assessment={kind:'ai_writing'|'ai_speaking';strengths?:string;examples?:{quote:string;explanation:string;better_answer:string}[];estimated:boolean;band:number;tasks:{position:number;criteria:Record<string,number>;feedback:string;evidence:string;improvement:string;band:number}[];model:string;note:string};
...
export type Attempt={assessment_status?:string;assessment_error?:string;id:string;title:string;section:string;state:string;deadline:string;server_time:string;started_at:string;answers:Record<string,string>;review_positions:number[];passage?:string;audio_url?:string;questions?:{position:number;prompt:string;choices:string[];skill_tag:string}[];result:Result|null};
